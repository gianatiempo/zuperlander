...
def dynaform_form(context, form_slug, success_url, form_template=None, *args, **kwargs):
    ...
    try:
        object_form = FormularioDinamico.objects.get_for_lang().get(slug=form_slug, is_active=True)
    except FormularioDinamico.DoesNotExist:
        return {mark_safe('<!-- FormularioDinamico %s DoesNotExist -->' % form_slug): []}
    ...
    if request.method == "POST":
        form = DynaFormClassForm(data=request.POST, files=request.FILES, request=request, object_form=object_form,
                                 context=context, success_url=success_url)
    else:
    ...
    if form_template:
        try:
            template = TemplateFormularioDinamico.objects.get(id=form_template)
            template = Template(template.html)
        except TemplateFormularioDinamico.DoesNotExist:
            return form
